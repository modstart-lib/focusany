            ...
            for (const m of a.matches) {
                if (m.type === ActionMatchTypeEnum.REGEX) {
                    ...
                    if ('filterExtensions' in m) {
                        files = files.filter(f => f.isFile && (m as ActionMatchFile).filterExtensions.includes(f.fileExt))
                    }
                    ...
                    if ((m as ActionMatchWindow).nameRegex && !ReUtil.match((m as ActionMatchWindow).nameRegex, activeWindow.name)) {
                        continue
                    }
                    if ((m as ActionMatchWindow).titleRegex && !ReUtil.match((m as ActionMatchWindow).titleRegex, activeWindow.title)) {
                        continue
                    }
