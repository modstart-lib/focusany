        ...
        const updateMagnifier = async () => {
            ...
            try {
                ...
                const offsetX = Math.round(
                    (mousePos.x - display.bounds.x) * scaleFactor + (originalPhysicalX - clampedPhysicalX),
                )
                const offsetY = Math.round(
                    (mousePos.y - display.bounds.y) * scaleFactor + (originalPhysicalY - clampedPhysicalY),
                )
                const colors: number[] = []
                ...
                const isMouseInTopRight =
                    mousePos.x > display.bounds.x + display.bounds.width * 0.75 &&
                    mousePos.y < display.bounds.y + display.bounds.height * 0.25
                let x, y
