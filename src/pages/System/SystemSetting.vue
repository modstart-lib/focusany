...
onMounted(() => {
    ...
    testActionSet('systemSetting.loaded', () => true)
})
onUnmounted(() => {
    testActionUnset('systemSetting.loaded')
})
...
<template>
    <div class="p-4">
        <div class="mb-8">
            <div class="text-base font-bold mb-4">{{ $t('setting.functionSettings') }}</div>
            <div class="pl-4">
                <div class="flex items-center mb-6">
                    <div class="flex-grow">{{ $t('setting.language') }}</div>
                    <div>
                        <a-select :model-value="i18n.global.locale.value" @change="changeLocale($event as string)">
                            <a-option v-for="item in listLocales()" :key="item.name" :value="item.name">{{
                                item.label
                            }}</a-option>
                        </a-select>
                    </div>
                ...
                <div class="flex items-center mb-6">
                    <div class="flex-grow">{{ $t('setting.invokeHotkey') }}</div>
                    <div>
                        <HotkeyInput
                ...
                <div class="flex items-center mb-6">
                    ...
                    <div>
                        ...
                        >
                            <a-radio value="light">{{ $t('setting.lightTheme') }}</a-radio>
                            <a-radio value="dark">{{ $t('setting.darkTheme') }}</a-radio>
                ...
                <div class="flex items-center mb-6" v-if="setting.configGet('fastPanelEnable', true).value">
                    <div class="flex-grow">{{ $t('setting.fastPanelHotkey') }}</div>
                    <div>
                        <a-select
                            :model-value="fastPanelTriggerType"
                            @change="onManagerConfigChange('fastPanelTriggerType', $event)"
                        >
                            <a-option value="Ctrl" v-if="isWindows || isLinux">{{
                                $t('setting.ctrlSingleClick')
                            }}</a-option>
                            <a-option value="Ctrl" v-else-if="isMacOs">{{ $t('setting.controlSingleClick') }}</a-option>
                            <a-option value="Ctrl:2" v-if="isWindows || isLinux">{{
                                $t('setting.ctrlDoubleClick')
                            }}</a-option>
                            ...
                            <a-option value="Alt" v-else-if="isMacOs">{{ $t('setting.optionSingleClick') }}</a-option>
                            <a-option value="Alt:2" v-if="isWindows || isLinux">{{
                                $t('setting.altDoubleClick')
                            }}</a-option>
                            <a-option value="Alt:2" v-else-if="isMacOs">{{ $t('setting.optionDoubleClick') }}</a-option>
                            <a-option value="Meta" v-if="isMacOs">{{ $t('setting.commandSingleClick') }}</a-option>
                            <a-option value="Meta:2" v-if="isMacOs">{{ $t('setting.commandDoubleClick') }}</a-option>
                        </a-select>
                    </div>
                </div>
                <div class="flex items-center mb-6">
                ...
                <div class="flex items-center mb-6">
                    <div class="flex-grow">{{ $t('setting.detachWindowHotkey') }}</div>
                    <div>
                        <HotkeyInput
                            :value="manager.configGet('detachWindowTrigger', null)"
