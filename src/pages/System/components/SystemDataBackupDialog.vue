...
<template>
    <a-drawer :width="500" :visible="visible" @close="onClose" @ok="onClose" @cancel="onClose" unmountOnClose>
        ...
        <template #footer>
            <a-button size="small" @click="onClose()"> {{ $t('common.close') }} </a-button>
        </template>
        <div style="margin: -12px -16px">
            <div class="p-3">
                <a-radio-group type="button" v-model:model-value="type">
                    <a-radio value="backup">{{ $t('backup.backupToFile') }}</a-radio>
                    <a-radio value="restore">{{ $t('backup.restoreFromFile') }}</a-radio>
